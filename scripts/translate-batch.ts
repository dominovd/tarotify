...
function parseArgs(argv: string[]) {
  ...
  if (!locale) {
    console.error('Usage: tsx scripts/translate-batch.ts <locale> [--force] [--slugs=a,b] [--dry-run]')
    process.exit(1)
  }
  ...
  return { locale, force, dryRun, slugs }
}
...
async function main() {
  const { locale, force, dryRun, slugs: onlySlugs } = parseArgs(process.argv.slice(2))
  ...
  const requests: BatchRequest[] = pending.map(slug => {
    const payload: Record<SourceFile, unknown> = {} as any
    for (const file of CARD_SOURCE_FILES) {
      if (source[file][slug]) payload[file] = source[file][slug]
    }
    return {
      custom_id: `${locale}:${slug}`,
      params: {
        model: MODEL,
  ...
  const client = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY }) as unknown as AnthropicClient
  ...
  while (status !== 'ended') {
    ...
    console.log(`  batch ${batch.id} status: ${status}`)
  }
  ...
  for await (const item of client.messages.batches.results(batch.id)) {
    ...
    const text = item.result.message.content.map(c => c.text).join('')
    const slug = item.custom_id.split(':').slice(1).join(':')
    try {
