...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        'en': `https://tarotaxis.com/cards/${card.slug}/reversed`,
        'es': `https://tarotaxis.com/es/cartas/${esSlug}/invertida`,
        'x-default': `https://tarotaxis.com/cards/${card.slug}/reversed`,
      },
    },
    openGraph: {
    ...
    <div style={{ maxWidth: 760, margin: '0 auto', padding: '2rem 1.5rem 5rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: '.75rem', marginBottom: '2.5rem' }}>
        ...
        <Link href={`/cards/${card.slug}/feelings`} style={{ display: 'block', padding: '1rem 1.1rem', background: 'rgba(201,168,76,.06)', border: '1px solid rgba(201,168,76,.25)', borderRadius: 12, textDecoration: 'none' }}>
          ...
          <div style={{ color: 'var(--text)', fontSize: '.88rem' }}>❦ {card.name} as Feelings →</div>
        </Link>
