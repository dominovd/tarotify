...
export async function GET(request: Request) {
  try {
    ...
    // Pre-fetch card image as base64 to avoid self-referential edge fetch issues
    let imgSrc: string = `${base}/cards/${card.slug}.webp`
    try {
      const res = await fetch(`${base}/cards/${card.slug}.webp`)
      if (res.ok) {
    ...
    // Return the error as plain text so we can debug
