...
export default function SignInForm({ next, initialError, initialSent, mode = 'signin' }: Props) {
  ...
  const nextParam = next ?? searchParams?.get('next') ?? '/es/account'
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    // Pass `next=/es/...` to the shared callback so the user lands back on the
    // Spanish tree after clicking the magic link.
    ...
    return (
      <div style={{
        ...
        <p style={{ color: 'var(--muted)', fontSize: '.88rem', lineHeight: 1.7, margin: 0 }}>
          Te enviamos un enlace mágico a <strong style={{ color: 'var(--text)' }}>{email || 'tu correo'}</strong>.
          Haz clic en el enlace desde este dispositivo para iniciar sesión. El enlace expira en 1 hora.
        </p>
      </div>
    )
  }
  ...
  return (
    <form onSubmit={onSubmit}>
      ...
      >
        <GoogleGlyph />
        {googleLoading ? 'Redirigiendo…' : (mode === 'signup' ? 'Crear cuenta con Google' : 'Continuar con Google')}
      </button>
      ...
      >
        {sending ? 'Enviando…' : mode === 'signup' ? 'Crear cuenta' : 'Enviar enlace mágico'}
      </button>
    </form>
  )
}
